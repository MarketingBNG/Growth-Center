/**
 * The metrics themselves: the funnel, the pipeline valuation, the channel and account
 * figures, the operational medians, and the trend series.
 *
 * Was the bulk of one 1,772-line file. The per-screen KPI builders that assemble these
 * into cards live in kpis.ts, and the window arithmetic underneath them in window.ts;
 * lib/metrics.ts re-exports all three, so no import anywhere else changes.
 */
import {
  consultationHeld,
  newCustomer,
...
export async function comparableDeltas(cards: Kpi[], current: Range, previous: Range): Promise<Kpi[]> {
...
export async function sessions(range: Range): Promise<number> {
 ...
 */
export async function convertedLeads(range: Range): Promise<number> {
