...
export async function buildReport(id: ReportId, days: number): Promise<Report> {
  ...
  if (id === 'sales') {
    ...
    const [stages, won, lost, revenue, pipeline] = await Promise.all([
      db().pipelineStage.findMany({
        ...
        include: { opportunities: { where: { closedAt: null }, select: { value: true, probability: true } } },
      }),
      db().opportunity.findMany({ where: { closedAt: window, stage: { isWon: true } }, select: { value: true } }),
      db().opportunity.findMany({ where: { closedAt: window, stage: { isLost: true } }, select: { value: true, lostReason: true } }),
      ...
      sections: [
        {
          ...
          rows: [
            { label: 'Won', value: int(won.length), hint: money(won.reduce((t, o) => t + Number(o.value), 0)) },
            { label: 'Lost', value: int(lost.length), hint: money(lost.reduce((t, o) => t + Number(o.value), 0)) },
            { label: 'Win rate', value: won.length + lost.length ? `${((won.length / (won.length + lost.length)) * 100).toFixed(0)}%` : '—' },
            { label: 'Revenue booked', value: money(Number(revenue._sum.amount ?? 0)) },
            { label: 'Open pipeline', value: money(pipeline.total), hint: `${money(pipeline.weighted)} weighted` },
          ],
        },
        {
          ...
          rows: stages
            ...
            .map((s) => {
              const value = s.opportunities.reduce((t, o) => t + Number(o.value), 0);
              const weighted = s.opportunities.reduce((t, o) => t + (Number(o.value) * o.probability) / 100, 0);
  ...
  const [byChannel, byCampaign, direct] = await Promise.all([
    db().revenueEntry.groupBy({ by: ['channelId'], where: { date: window }, _sum: { amount: true }, _count: { _all: true } }),
    db().revenueEntry.groupBy({ by: ['campaignId'], where: { date: window }, _sum: { amount: true } }),
    db().revenueEntry.aggregate({ where: { date: window, channelId: null }, _sum: { amount: true } }),
  ]);
  ...
  const totalRevenue = byChannel.reduce((t, r) => t + Number(r._sum.amount ?? 0), 0);
      ...
      },
      {
        ...
        rows: byChannel
          ...
          .map((r) => {
            ...
            return [
              r.channelId ? (channelName.get(r.channelId) ?? 'Unknown') : 'No channel recorded',
              int(r._count._all),
              money(amount),
              totalRevenue ? `${((amount / totalRevenue) * 100).toFixed(1)}%` : '—',
            ];
          }),
      },
      {
        ...
        rows: byCampaign
          ...
          .sort((a, b) => Number(b._sum.amount ?? 0) - Number(a._sum.amount ?? 0))
          .map((r) => [
            r.campaignId ? (campaignName.get(r.campaignId) ?? 'Unknown') : 'No campaign recorded',
            money(Number(r._sum.amount ?? 0)),
          ]),
      },
      {
        kind: 'stats',
        title: 'Unattributed',
        rows: [{ label: 'Revenue with no channel', value: money(Number(direct._sum.amount ?? 0)) }],
      },
    ],
  };
}
